...
from ..common import FCEncoder, ConvEncoder, DiscreteHead, DuelingHead, DuelingHeadM, MultiHead, RainbowHead, \
...
@MODEL_REGISTRY.register('dqn_ma')
class MADQN(nn.Module):
    ...
    def __init__(
        ...
        for _ in range(self.agent_num):
            self.heads.append(deepcopy(self.head))
    ...
    def forward(self, x: torch.Tensor) -> list:
        ...
        for agent in range(self.agent_num):
            tmp = self.encoders[agent](x)
            tmp = self.heads[agent](tmp)
            output.append(tmp)
        return output

@MODEL_REGISTRY.register('dqn_ma_share_backbone')
class MADQNSB(nn.Module):
    ...
    def __init__(
        ...
        if multi_head:
            ...
            for _ in range(self.ensemble_num):
                # we have different initilization for each head
                self.heads.append(head_cls(
                    head_hidden_size, action_shape, head_layer_num, activation=activation, norm_type=norm_type
                ))
        # the same initilization
        # for _ in range(self.ensemble_num):
    ...
    def forward(self, x: torch.Tensor) -> list:
        ...
        output = []
        for agent in range(self.ensemble_num):
            tmp = self.encoder(x)
            tmp = self.heads[agent](tmp)
            output.append(tmp)
        # list of dict: [{'logit':}, {'logit':},...]
        return output

@MODEL_REGISTRY.register('dqn_ma_share_backbone_head')
class MADQNSBH(nn.Module):
    ...
    def __init__(
        ...
        if multi_head:
            ...
            self.head = head_cls(
                head_hidden_size, action_shape, head_layer_num, activation=activation, norm_type=norm_type,  ensemble_num=ensemble_num,
            )
