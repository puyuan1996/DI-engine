...
from ding.utils import RunningMeanStd, deep_merge_dicts, SequenceType, REWARD_MODEL_REGISTRY
from .base_reward_model import BaseRewardModel
import os
import yaml
from dizoo.gfootball.model.q_network.football_q_network import ScalarEncoder, PlayerEncoder, SpatialEncoder

with open(os.path.join(os.path.dirname(__file__), '/home/puyuan/DI-engine/dizoo/gfootball/model/q_network/football_q_network_default_config.yaml')) as f:
...
@REWARD_MODEL_REGISTRY.register('rnd-ngu-gfootball')
class RndNGUGfootballRewardModel(BaseRewardModel):
    r"""
    Overview:
    ...
    def __init__(self, config: EasyDict, device: str, tb_logger: 'SummaryWriter') -> None:  # noqa
        super(RndNGUGfootballRewardModel, self).__init__()
        self.cfg = config
        ...
        self.cfg_gfootball = deep_merge_dicts(football_q_network_default_config.model, cfg)
        scalar_encoder_arch = self.cfg_gfootball.encoder.match_scalar
        player_encoder_arch = self.cfg_gfootball.encoder.player
        ...
        self.player_encoder.to(self.device)
        self.scalar_encoder.to(self.device)


        self.encoder = self.football_obs_encoder
        self.opt = optim.Adam([{"params": self.reward_model.predictor.parameters()}, {"params":self.scalar_encoder.parameters()}, {"params":self.player_encoder.parameters()}], config.learning_rate)
    ...
    def _train(self) -> None:
        ...
        # # for gfootball
        # for bs in range(self.cfg.batch_size):
        #     train_data[bs] = self.encoder(train_data[bs]).view(-1)
    ...
    def train(self) -> None:
        ...
        if self.only_use_last_five_frames:
            ...
            # self.train_data_cur = self.train_data_total

            # another implementation way
            # tmp = []
            # for i in range(len(self.train_data_total)):
            #     tmp += self.train_data_total[i]
            # self.train_data_cur = tmp
...
@REWARD_MODEL_REGISTRY.register('episodic-ngu-gfootball')
class EpisodicNGUGfootballRewardModel(BaseRewardModel):
    r"""
    Overview:
    ...
    def __init__(self, config: EasyDict, device: str, tb_logger: 'SummaryWriter') -> None:  # noqa
        super(EpisodicNGUGfootballRewardModel, self).__init__()
        self.cfg = config
        assert device == "cpu" or device.startswith("cuda")
        self.device = device
        self.tb_logger = tb_logger
        ...
        self.scalar_encoder.to(self.device)
        self.encoder = self.football_obs_encoder
        self.opt = optim.Adam([{"params":self.episodic_reward_model.parameters()}, {"params":self.scalar_encoder.parameters()}, {"params":self.player_encoder.parameters()}], config.learning_rate)
    ...
    def train(self) -> None:
        ...
        B = len(self.train_obs_total)
        T = len(self.train_obs_total[0])
        self.train_next_obs_total=[[] for bs in range(B)]
        for bs in range(B):
            for timestep in range(T):
                self.train_next_obs_total[bs].append(self.train_obs_total[bs][timestep].clone())
