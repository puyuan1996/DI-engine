...
@POLICY_REGISTRY.register('dqn-cluster')
class DQNClusterPolicy(Policy):
    r"""
    Overview:
    ...
    def _forward_learn(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        data_n = q_nstep_td_data(
            q_value, target_q_value, data['latent_action'], target_q_action, data['reward'], data['done'], data['weight']
        )
    ...
    def _forward_collect(self, data: Dict[int, Any], eps: float) -> Dict[int, Any]:
        ...
        if self._cfg.model.action_shape == 4:
            action_mapping = torch.tensor([[-0.6066874,  -0.44333179],
                [ 0.6088612,  0.52264505],
                [ 0.64009869, -0.43649803],
                [-0.58009607,  0.32221041]])
        elif self._cfg.model.action_shape == 8:
            action_mapping = torch.tensor([[ 0.72485906,  0.03408122],
                [-0.50984142,  0.69521874],
                [ 0.03377901,  0.19940135],
                [ 0.64852572,  0.69086607],
                [-0.71777,    -0.55843774],
                [-0.02521474, -0.61822142],
                [-0.70700778,  0.02654735],
                [ 0.739567,   -0.6200412 ]])
        # here output['action'] is the out of DQN, is discrete action
        output['latent_action'] = copy.deepcopy(output['action'])
        # use the cluster center of SAC expert action
        output['action'] = torch.stack([action_mapping[int(action_index)] for action_index in output['action']])
