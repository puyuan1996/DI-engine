...
@POLICY_REGISTRY.register('dqn_ma')
class MADQNPolicy(Policy):
    ...
    def _init_learn(self) -> None:
        ...
        self._learn_model = model_wrap(self._model, wrapper_name='multi_argmax_sample')
        self._learn_model.reset()
        self._target_model.reset()
        self.disc_to_cont = list(product(*[list(range(self._cfg.model.action_shape)) for dim in range(self._cfg.model.agent_num)] ))
    ...
    def _forward_learn(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self._cuda:
            data_action_list = to_device(data_action_list, self._device)
        for agent in range(self._cfg.model.agent_num): 
            # Current q value (main model)
            q_value = self._learn_model.forward(data['obs'])[agent]['logit']
            # Target q value
            with torch.no_grad():
                target_q_value = self._target_model.forward(data['next_obs'])[agent]['logit']
                # Max q value action (main model)
                target_q_action = self._learn_model.forward(data['next_obs'])[agent]['action']

            data_n = q_nstep_td_data(
                q_value, target_q_value, data_action_list[:, agent], target_q_action, data['reward'], data['done'], data['weight']
            )
    ...
    def _init_collect(self) -> None:
        ...
        self._collect_model = model_wrap(self._model, wrapper_name='multi_eps_greedy_sample')
    ...
    def _forward_collect(self, data: Dict[int, Any], eps: float) -> Dict[int, Any]:
        ...
        with torch.no_grad():
            ...
            # (1,2,0) --> int(1*(4**2) + 2*(4**1) + 0*(4**0)) = 24
            # output = int(output[0] * (4**2) + output[1] * (4**1) + output[2] * (4**0))
            ans = {}
            ans['action'] = torch.tensor([ int(sum(
                [output[action_dim]['action'][env_num] * (self._cfg.model.action_shape ** (self._cfg.model.agent_num - action_dim -1)) for action_dim in range(self._cfg.model.agent_num)]
                )) for env_num in range(output[0]['action'].shape[0]) ])
            output = ans
        if self._cuda:
    ...
    def _init_eval(self) -> None:
        ...
        """
        self._eval_model = model_wrap(self._model, wrapper_name='multi_argmax_sample')
    ...
    def default_model(self) -> Tuple[str, List[str]]:
        ...
        return 'dqn_ma', ['ding.model.template.q_learning']
