...
def train(args):
    ...
    env_steps = [0, 100000, 200000, 500000, 1000000, 1500000]
    ...
    plt.plot(env_steps, iter_noop_action_ratio_list_dqn_cluster_k8, 's-', label="DQN + expert cluster")  # s-:方形
    plt.plot(env_steps, iter_noop_action_ratio_list_adq_k8, 's-', label="ADQ")  # s-:方形
    ...
    plt.legend(loc="best", fontsize=size2)  # 显示图例
    ...
    # plt.savefig(f'{visualize_path}' + f'20eps_statistics_Noop_Action_Ratio_v1.png')
    plt.savefig(f'{visualize_path}' + f'20eps_statistics_Noop_Action_Ratio_1.5M_legend16_fs7-5.5.pdf')
    ...
    ax.plot(env_steps, iter_total_fuel_cnt_list_dqn_cluster_k8, 'o-', color='g', label="DQN + expert cluster")  # o-:圆形
    ax.plot(env_steps, iter_total_fuel_cnt_list_adq_k8, 'o-', color='b', label="ADQ-K8")  # o-:圆形

    plt.xlabel("Env Steps")  # 横坐标名字
    plt.ylabel("Average Total Fuel Count")  # 纵坐标名字
    plt.legend(loc="best")  # 图例
    plt.savefig(f'{visualize_path}' + f'20eps_statistics_Total_Fuel_Count_v1.png')
